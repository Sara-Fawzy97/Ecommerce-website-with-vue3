...
<div class=" lg:flex lg:flex-row-reverse">
   <img :src=" SingleProduct.thumbnail " :alt="SingleProduct.title" class="h-[300px] w-[480px] md:h-[390px]">
...
<div class="text-start">
   ...
   <div class="flex gap-2 mt-5">
     <div class=" flex border border-gray border-solid rounded-[10px] p-2 md:py-3 md:px-5">
      ...
      <button @click='addToCart(SingleProduct)' class=" md:text-[20px] hover:text-white hover:bg-primary-color px-5 py-3 border border-solid hover:border-0  border-black rounded-[15px]">Add To Cart</button>
...
<div class="mt-7">
   ...
   <!------reviews Section ---->
   <div  class="p-7 md:p-20 text-start ">
      <table class="w-full">
         <tr  v-for="review in SingleProduct.reviews"  :key="review.id">
            <td class="md:p-5">
             <h5 class="bold text-[12px] md:text-base ">{{ review.reviewerName}}</h5>
            </td>
            <td class="p-5 flex justify-between">
               <div>
               <p class="text-gray text-[12px]  md:text-base">{{ review.comment }}</p>
               <span class="md:text-xs text-[10px]">{{ review.date.substring(0,10) }}</span>
            </div>
               <div class=" flex">
         <StarIcon v-for="rating in [0, 1, 2, 3, 4]" :key="rating" :class="[review.rating > rating ? 'text-primary-color' : 'text-gray-200', 'size-4 md:size-5 shrink-0']" aria-hidden="true" />
      </div>
            </td>
         </tr>
