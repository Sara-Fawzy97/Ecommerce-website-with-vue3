  ...
  <div class="bg-[#F9F1E7]">
    <div class="flex justify-between py-5 px-16">
      <div class="flex justify-between hover:cursor ">
        ...
        <div>
        <p class="categDiv relative ">Shop by category</p>
      <div class=" allCategories bg-white/80  grid grid-cols-4 gap-3 p-4 rounded absolute z-40 mt-1 ">
         <router-link :to="'categories/'+category.slug" class="hover:bg-gray/70 p-1 rounded cursor-pointer" v-for="category in categories" :key="category.slug">
        {{ category.name }}
      </router-link></div>  
      </div>
      </div>
